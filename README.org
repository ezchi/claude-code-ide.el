...
**** vterm Native Prompt Tracking

To ensure the most reliable terminal interaction (especially for =C-c '= and allowing Claude to see what you are currently typing), it is highly recommended to enable **vterm's native prompt tracking**.

1. In your Emacs configuration:
   #+begin_src emacs-lisp
   (setq vterm-use-vterm-prompt-detection-method t)
   #+end_src

2. In your shell configuration (e.g., =~/.bashrc= or =~/.zshrc=), add the required escape sequence.
...
Refer to the [[https://github.com/akermu/emacs-libvterm#shell-configuration][vterm documentation]] for more details on shell configuration.
